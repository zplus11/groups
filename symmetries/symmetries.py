...
class dihedral:
    ...
    def subgroups(self):
        ...
        for sublist in sublists_to_check:
            if len(self.symmetries) % len(sublist) == 0:
                if self.__check_subgroup(sublist):
    ...
    def __reflect_from_v(self, vertex: tuple = (1, )):
        temp = self.__temp()
        base = vertex[0]
    ...
    def __reflect_from_m_to_m(self, vertex: tuple = (1, 2)):
        temp = self.__temp()
        base = vertex[0]
    ...
    def __check_subgroup(self, subset: list):
        if 0 not in subset:
            return False
        elif any(self.__determine(self.apply(i, j)) not in subset for i in subset for j in subset):
            return False
        else:
            return True
